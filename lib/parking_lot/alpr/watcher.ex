defmodule ParkingLot.ALPR.Watcher do
  ...
  def handle_cast({:register, recognition}, %{type: type} = state) do
    action = Keyword.get([internal: :leave, external: :entry], type)

    with {:ok, vehicle} <- Customers.get_vehicle(license_plate: recognition),
         {:ok, parking} <- Parkings.register_parking(action, vehicle) do
