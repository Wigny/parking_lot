defmodule ParkingLotWeb.UserAuth do
  @moduledoc false
  ...
  def log_out_user(conn) do
    if user_token = get_session(conn, :user_token) do
      Accounts.delete_sessions(token: user_token)
    end

    if live_socket_id = get_session(conn, :live_socket_id) do
      ParkingLotWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_user(conn, _opts) do
    ...
    user = user_token && Accounts.get_user(session: [token: user_token])
  ...
  the current_user:
      ...
      defmodule ParkingLotWeb.PageLive do
        use ParkingLotWeb, :live_view

        on_mount {ParkingLotWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{ParkingLotWeb.UserAuth, :ensure_authenticated}] do
  ...
  defp mount_current_user(session, socket) do
    Phoenix.Component.assign_new(socket, :current_user, fn ->
      if user_token = session["user_token"] do
        Accounts.get_user(session: [token: user_token])
      end
    end)
  end
