defmodule ParkingLotWeb.CoreComponents do
  @moduledoc """
  ...
  attr :type, :string,
    ...
    values: ~w(checkbox color date datetime-local email file hidden month number password
               range radio search select tel text textarea time url week digits)
  ...
  attr :rest, :global, include: ~w(autocomplete cols disabled form max maxlength min minlength
                                   pattern placeholder readonly required rows size step inputmode
                                   pattern phx_hook)
  slot :inner_block
  ...
  def input(%{type: "digits"} = assigns) do
    assigns
    |> assign(:type, "text")
    |> assign(:rest, inputmode: "numeric", pattern: "[0-9]*", phx_hook: "InputPatternConstraint")
    |> input()
  end
  ...
  @doc """
  Formats given value for pattern

  ## Example
  <.mask value="04543413261" pattern={~r/(\d{3})(\d{3})(\d{3})(\d{2})/} format={~S"\1.\2.\3-\4"} />
  """
  attr :value, :string, required: true
  attr :pattern, :string, required: true
  attr :format, :string, required: true

  def mask(%{pattern: pattern, format: format} = assigns) do
    assigns = update(assigns, :value, fn value -> String.replace(value, pattern, format) end)
